defmodule KitchenSink.MapTest do
  ...
  test "map with 2 element produces the deep_merge of those 2 according to merge/2 rules" do
    ...
    assert KMap.deep_merge(%{a: %{b: 1, c: 1}}, %{a: %{b: 2}}) == %{
             a: %{b: 2, c: 1}
           }
  end

  test "map with many elements produces the deep merge of those 3" do
    assert KMap.deep_merge_list([%{a: 1}, %{b: 1}, %{c: 1}]) == %{
             a: 1,
             b: 1,
             c: 1
           }
    ...
    assert KMap.deep_merge_list([%{a: %{b: 1, c: 1}}, %{a: %{b: 2}}, %{d: 2}]) ==
             %{a: %{b: 2, c: 1}, d: 2}

    assert KMap.deep_merge_list([%{a: %{b: %{c: %{d: 1}}}}, %{a: %{z: 1}}]) ==
             %{a: %{b: %{c: %{d: 1}}, z: 1}}
  end

  test "map with list" do
    assert KMap.deep_merge(%{a: [%{b: 1}, %{c: 2}]}, %{a: [%{b: 3}, %{c: 4}]},
             merge_lists: true
           ) ==
             %{a: [%{b: 3}, %{c: 4}]}

    assert KMap.deep_merge(%{a: [%{b: 1}, %{c: 2}]}, %{a: [%{d: 3}, %{e: 4}]},
             merge_lists: true
           ) ==
             %{a: [%{b: 1, d: 3}, %{c: 2, e: 4}]}

    assert KMap.deep_merge(
             %{a: [%{b: 1}, %{c: 2}, %{f: 5}]},
             %{a: [%{d: 3}, %{e: 4}]},
             merge_lists: true
           ) ==
    ...
    assert KMap.deep_merge(%{a: []}, %{a: [%{d: 3}, %{e: 4}, %{f: 5}]},
             merge_lists: true
           ) ==
             %{a: [%{d: 3}, %{e: 4}, %{f: 5}]}
    ...
    assert KMap.deep_merge(%{a: nil}, %{a: [%{d: 3}, %{e: 4}, %{f: 5}]},
             merge_lists: true
           ) ==
             %{a: [%{d: 3}, %{e: 4}, %{f: 5}]}

    assert KMap.deep_merge(%{a: [%{b: 1}, %{c: 2}]}, %{a: nil},
             merge_lists: true
           ) ==
             %{a: nil}
  ...
  test "renaming a key that exists does nothing when overwrite: false" do
    assert KMap.rename_key(%{a: 1, b: 2}, :a, :b, overwrite: false) == %{
             a: 1,
             b: 2
           }
  ...
  test "new nested key" do
    ...
    actual =
      KMap.make_nested("kenbert this is for you", [:parent, :child, :conception])
  ...
  describe "Map.key_paths/1" do
    ...
    test "values that are empty maps" do
      ...
      assert %{key1: %{key1: %{}, key2: %{}}, key2: %{key1: %{}, key2: %{}}}
             |> KMap.key_paths() ==
